...
Player::Player(Graphics& graphics, const std::string& path_to_bmp, double pos_x, double pos_y) : Player::Player() {
	...
	SDL_Texture* player_texture = graphics.LoadTextureFromImage(path_to_bmp, SDL_Point{0, 0});
...
void Player::Update(const Input& input, double delta_time, Tilemap& tilemap)
{
	...
	if (status_ & (unsigned char)Status::GROUNDED)
	{
		...
		if (dpad_vector_[0] == 0)
		{
			...
			if (abs(vel_x_) < Physics::DECEL_GROUND_NORMAL * dt_ratio * dt_ratio)
			{
				vel_x_ = 0;
			}
			else
			{
				vel_x_ -= Physics::DECEL_GROUND_NORMAL * dt_ratio * dt_ratio * sgn(vel_x_);
			}
		}
	...
	if (delta_x > 0)
	{
		...
		std::vector<CollisionInfo> collisions = GetCollisionInfo(tilemap,
			RightCollision(std::max(0.0, delta_x)));
		if (!collisions.empty())
		{
		...
		std::vector<CollisionInfo> collisions = GetCollisionInfo(tilemap,
			LeftCollision(std::min(0.0, delta_x)));
		if (!collisions.empty())
		{
...
int Player::Draw(Graphics& graphics, double offset_x, double offset_y)
{
	...
	int return_code = sprite_->Draw(graphics, {floor(pos_x_ + offset_x),
		floor(pos_y_ + offset_y)}, flip);
	if (Game::debug_show_hitboxes)
	{
		graphics.DrawColoredRect(
			{floor(pos_x_) - 8 + offset_x, floor(pos_y_) - 16 + offset_y, 16, 16},
			{0x00, 0x00, 0x00, 0x40});
		graphics.DrawColoredRect(
			RightCollision(0) + Rectangle(offset_x, offset_y, 0, 0 ),
			SDL_COLOR_WHITE);
		graphics.DrawColoredRect(
			LeftCollision(0) + Rectangle(offset_x, offset_y, 0, 0),
			SDL_COLOR_WHITE);
		graphics.DrawColoredRect(
			BottomCollision(0) + Rectangle(offset_x, offset_y, 0, 0),
			SDL_COLOR_WHITE);
		graphics.DrawColoredRect(
			TopCollision(0) + Rectangle(offset_x, offset_y, 0, 0),
			SDL_COLOR_WHITE);
	}
	return return_code;
}
...
std::vector<Player::CollisionInfo> Player::GetCollisionInfo(const Tilemap& tilemap,
